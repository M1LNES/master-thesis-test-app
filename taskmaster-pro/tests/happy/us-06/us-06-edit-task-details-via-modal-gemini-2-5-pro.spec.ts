...
test.describe("[US-06] Edit Task details via Modal", () => {
  test("should allow a user to edit a task priority and see the update on the dashboard", async ({
    page,
  }) => {
    ...
    await expect(editModal.getByLabel("Title")).toHaveValue(
    ...
    // Bonus: Verify success notification
    ...
    await expect(taskCardLocator).toBeVisible();
